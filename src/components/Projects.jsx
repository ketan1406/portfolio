  ...
  return (
    <motion.div variants={fadeIn("up", "spring")}>
      <Tilt
        ...
        className="bg-tertiary p-4 rounded-2xl w-full xs:w-[180px] sm:w-[360px]"
      >
        {/* Image Container */}
        <div className="relative w-full xs:h-[115px] sm:h-[230px]">
          <img
            src={image}
            alt={name}
        ...
        {/* Project Details */}
        <div className="mt-4">
          <h3 className="text-white font-bold xs:text-[10px] sm:text-[20px]">{name}</h3>
          <p className="mt-2 text-secondary xs:text-[7px] sm:text-[14px]">{description}</p>
        </div>

        {/* Tags */}
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <p key={tag.name} className={`xs:text-[6px] sm:text-[12px] ${tag.color}`}>
              #{tag.name}
            </p>
          ))}
        </div>
